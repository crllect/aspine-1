...
let classIndex = function (classname) {
    let classesArray = currentTableData.currentTermData.classes.map(
        (x) => x.name
    );
    ...
    if (isNaN(numberGrade)) {
        ...
        if (edited) {
            fake =
                ...
                calculated_grade +
                "% " +
                getLetterGrade(calculated_grade);
        }
        ...
        if (numberGrade >= 100 && calculated_grade >= 100) {
            ...
            return (
                ...
                "<br>" +
                "<span style='color:" +
                getColor(parseFloat(calculated_grade)) +
                "; font-weight:bold;'>" +
                fake +
                "</span>"
            );
  ...
  and the selected item: */
    ...
    if (term_dropdown_active) {
        ...
        for (i = 0; i < s.length; i++) {
            ...
            // tooltip text
            if (
                s.options[i].childNodes[0].nodeValue ===
                ...
                if (
                    typeof currentTableData.terms[currentTerm].classes ===
                    "undefined"
                ) {
                    // if (anyEdited()) {
                    // $(".gpa_select-selected").css('padding', "5px 16px 5px 16px");
                    // } else {
                    $(".gpa_select-selected").css(
                        "padding",
                        "13px 16px 13px 16px"
                    );
                    // }
                    ...
                    const year =
                        currentTableDataIndex === 0 ? "current" : "previous";
                    fetch("/data", {
                        method: "POST",
                        headers: {
                    ...
                    y = target.parentNode.getElementsByClassName(
                        "same-as-selected"
                    );
                    ...
                    if (i === 0) {
                        currentTableData.currentTermData =
                            currentTableData.terms.current;
                    } else {
                        currentTableData.currentTermData =
                            currentTableData.terms["q" + (i - 1)];
                    }

                    classesTable.setData(
...
let setup_quarter_dropdown = function () {
    ...
    if (!isNaN(currentTableData.terms.current.GPA.percent)) {
        document.querySelector("#current").textContent = document.querySelector(
            "#init_gpa"
        ).textContent = document.querySelector("#current_gpa").textContent =
            "Current Quarter GPA: " +
            currentTableData.currentTermData.GPA.percent;
    } else {
        document.querySelector("#current").textContent = document.querySelector(
            "#init_gpa"
        ).textContent = document.querySelector("#current_gpa").textContent =
    ...
    for (const term of termConverter) {
        ...
        if (!/q\d/.test(term)) continue;
        if (!isNaN(currentTableData.terms[term].GPA.percent)) {
            document.querySelector(
                `#${term}`
            ).textContent = document.querySelector(
                `#${term}_gpa`
            ).textContent = `Q${term[1]} GPA: ${currentTableData.terms[term].GPA.percent}`;
        } else {
            document.querySelector(
                `#${term}`
            ).textContent = document.querySelector(
                `#${term}_gpa`
            ).textContent = `Q${term[1]} GPA: None`;
        }
    }

    // Set up Yearly GPA
    document.querySelector("#cum").textContent = document.querySelector(
        "#cum_gpa"
    ).textContent = `Yearly GPA: ${currentTableData.cumGPA.percent}`;

    // Set text for currently selected quarter to be that of the corresponding
    // option
    document.querySelector(
        ".gpa_select-selected"
    ).textContent = document.querySelector("#gpa_select").options[
        termConverter.indexOf(currentTerm) + 1
    ].textContent;
};
...
let tableData_option_onclick = function () {
    ...
    let index_temp = parseInt(
        this.id.substring("tableData_select-items-".length)
    );
    currentTableDataIndex = isNaN(index_temp)
  ...
  and the selected item: */
    ...
    switch (currentTableData.type) {
        ...
        default:
            console.error(
                `Invalid currentTableData type ${currentTableData.type}`
            );
    }
    // Switch to this term
...
let initialize_tableData_dropdown = function () {
    ...
    for (i = 0; i < x.length; i++) {
        ...
        b.setAttribute(
            "class",
            "tableData_select-items select-items select-hide"
        );
      ...
      and open/close the current select box: */
            ...
            this.nextSibling.classList.toggle("select-hide");
            this.classList.toggle("select-arrow-active");
            $(".tableData_select-selected").toggleClass(
                "activated-selected-item"
            );
            $(".tableData_select-items div").toggleClass(
                "activated-select-items"
            );
        });
    }
};
...
let initialize_dayOfWeek_dropdown = function () {
    ...
    selElmnt = document
        .getElementById("day_custom-select")
    ...
    document.addEventListener("click", function () {
        document
            .getElementsByClassName("day_select-items")[0]
            .classList.add("select-hide");
        document
            .getElementById("day_select_div")
            .classList.remove("day_select-arrow-active");
    });
};

clearCount = 0;

window.onload = function () { // TODO: have a smarter way of automatically changing the currently selected day in the schedule
    setTimeout(function () {
        ...
        if (scheduleDiv) {
            ...
            // Simulate a click on the current day if it's a weekday
            if (currentDay > 0 && currentDay < 6) {

                let dayElement = document.getElementById(
                    `day_select-items-${currentDay}`
                );
                ...
                let checkInterval = setInterval(function () {
                    ...
                    if (dayElement) {
                        ...
                        clearCount++;
                        if (clearCount > 5) {
                            clearInterval(checkInterval); // clear after it works a couple times, sometimes it will break after working
                        }
                    }
                }, 1000);
            }
...
function setup_tooltips() {
    ...
    for (const child of document.querySelectorAll("[tooltip]")) {
        if (child.querySelector(".tooltiptext") === null) {
            ...
            node.appendChild(
                document.createTextNode(child.getAttribute("tooltip"))
            );
            ...
            if (child.hasAttribute("tooltip-margin")) {
                tooltiptext.style.marginLeft = child.getAttribute(
                    "tooltip-margin"
                );
            ...
            !child
                ...
                .classList.contains("resize-exempt")
        ) {
...
function parseTableData({ classes, quarter_oid }) {
    for (let i = 0; i < classes.length; i++) {
        ...
        for (let j = 0; j < (classes[i].assignments || []).length; j++) {
            ...
            classes[i].assignments[j].percentage =
                Math.round(
                    (classes[i].assignments[j].score /
                        classes[i].assignments[j].max_score) *
                        1000
                ) / 10;
            //initialize how the assignment should be colored in the table; based on percentage
            classes[i].assignments[j].color = getColor(
                classes[i].assignments[j].percentage
            );
            ...
            if (isNaN(classes[i].assignments[j].score)) {
                ...
                if (classes[i].assignments[j].special) {
                    ...
                    if (
                        ("" + classes[i].assignments[j].special).includes(
                            "("
                        ) &&
                        ("" + classes[i].assignments[j].special).includes(")")
                    ) {
                        ...
                        classes[i].assignments[j].score = regExp.exec(
                            classes[i].assignments[j].special
                        )[1];
                        classes[i].assignments[j].max_score = regExp.exec(
                            classes[i].assignments[j].special
                        )[1];
                    } else {
        ...
        if (classes[i].grade != "") {
            ...
            let gradeInfo = determineGradeType(
                computingClassData.assignments,
                computingClassData.categories,
                computingClassData.grade
            );
            //populating categoryDisplay which is the object used to display the category grading information
            classes[i].categoryDisplay = getCategoryDisplay(
                gradeInfo,
                computingClassData
            );
            ...
            classes[i].calculated_grade = computeGrade(
                computingClassData.assignments,
                computingClassData.categories,
                computingClassData.decimals,
                computingClassData.init_calculated_grade,
                computingClassData.grade
            ).categoryPercent;
...
let anyEdited = function () {
    let termsEdited = currentTableData.terms[currentTerm].classes.map(function (
        currentValue,
        index,
        array
    ) {
